...
class SocieteCommandesController extends Controller
{
    ...
    public function getOneCommande(Request $request, $id)
    {
        try {
            $user = Auth::user();
            if (!$user || !$user->id_hotel) {
            ...
            $commande = SocieteCommande::with('client', 'agence', 'detailsCommandesProduits', 'detailsCommandesMenus', 'accomptes')->where('id_hotel', $user->id_hotel)
            ->where('id_commande', $id)->first();
            ...
            return response()->json([
                'success' => false,
                'message' => 'Une erreur est survenue lors de la récupération de la comande.',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
    ...
    public function DeleteDetailProduit(Request $request, $id)
    {
        try {
            $user = Auth::user();
            if (!$user || !$user->id_hotel) {
            ...
            $SocieteDetailsProduit = SocieteDetailsCommandesProduits::findOrFail($id);

            $SocieteDetailsProduit->delete();

            return response()->json([
                'success' => true,
                'message' => 'Produit effacé avec succès.',
            ], 200);
    ...
    public function DeleteDetailMenu(Request $request, $id)
    {
        try {
            $user = Auth::user();
            if (!$user || !$user->id_hotel) {
            ...
            $SocieteDetailsMenu = SocieteDetailsCommandesMenus::findOrFail($id);

            $SocieteDetailsMenu->delete();

            return response()->json([
                'success' => true,
                'message' => 'Menu effacé avec succès.',
            ], 200);
    ...
    public function CreateDetailProduit(Request $request)
    {
        try {
            $user = Auth::user();
            if (!$user || !$user->id_hotel) {
            ...
            $validator = Validator::make($request->all(), [
                'id_commande' => 'required|string|exists:societe_commande,id_commande',
    ...
    public function CreateDetailMenu(Request $request)
    {
        try {
            $user = Auth::user();
            if (!$user || !$user->id_hotel) {
            ...
            Log::error('Erreur lors de l\'ajout des menus', [
                'error' => $e->getMessage(),
            ]);

            return response()->json([
                'success' => false,
                'message' => 'Une erreur est survenue lors de l\'ajout des menus.',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
