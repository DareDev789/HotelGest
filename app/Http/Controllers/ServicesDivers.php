...
class ServicesDivers extends Controller
{
    ...
    public function DeleteserviceDivers(Request $request, $id)
    {
        try {
            $user = Auth::user();
            ...
            Log::error('Une erreur est survenue lors de l\'effacement du service divers', ['error' => $e->getMessage()]);

            return response()->json([
                'message' => 'Une erreur est survenue lors de l\'effacement du service divers.',
                'error' => $e->getMessage(),
            ], 500);
        }
    }

    public function CreateserviceDivers(Request $request)
    {
        try {
            $user = Auth::user();
            ...
            if ($user && $user->id_hotel) {
                ...
                $serviceDivers=SocieteServicesDivers::create([
                    'designation' => $request->designation,
                    'description' => $request->description,
                    'prixPax'=> $request->prixPax,
            ...
            Log::error('Une erreur est survenue lors de l\'ajout du service divers', ['error' => $e->getMessage()]);

            return response()->json([
                'message' => 'Une erreur est survenue lors de l\'ajout du service divers.',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
