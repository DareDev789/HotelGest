...
class AccomptesController extends Controller
{
    ...
    public function createAccompteCommandes(Request $request)
    {
        try {
            $user = Auth::user();
            ...
            // Enregistrement du nouvel acompte
            $accompte = SocieteAccomptesCommandes::create([
    ...
    public function updateAccompteCommandes(Request $request, $id)
    {
        try {
            $user = Auth::user();
            ...
            // Récupérer l'acompte
            $accompte = SocieteAccomptesCommandes::find($id);
    ...
    public function PaidAccompteCommandes(Request $request, $id)
    {
        try {
            $user = Auth::user();
            ...
            $totalAccomptePaid = SocieteAccomptesCommandes::where('id_commande', $accompte->id_commande)
                ->sum('montant');

            // Vérifier si la somme payée couvre la totalité de la réservation
            if ($totalAccomptePaid >= $request->sommeAPayer) {
                SocieteCommande::where('id_commande', $accompte->id_commande)->update([
                    'statut_reservation' => 'oui',
                ]);
            }else{
                SocieteCommande::where('id_commande', $accompte->id_commande)->update([
